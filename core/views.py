...
def index(request):
    ...
    user_following = Follower_count.objects.filter(follower=request.user.username)
    ...
    return render(request, 'index.html', {'user_profile':user_profile, 'posts':feed_lists,'suggetions_username_profile_list': suggetions_username_profile_list[:4]})
...
def profile(request, pk):
    user_objek = User.objects.get(username=pk)
